...
import com.example.SoftwareProject.dto.UserDTO;
import com.example.SoftwareProject.model.UserEntity;
import com.example.SoftwareProject.security.TokenProvider;
...
public class UserController {
    ...
    public ResponseEntity<?> registerUser(@RequestBody UserDTO userDTO){
        try{
            ...
            //사용자 정보는 항상 하나이므로 리스트로 만들어야 하는 ResponseDTO를 사용하지 않고 그냥 UserDTO를 리턴
    ...
    public ResponseEntity<?> authenticate(@RequestBody UserDTO userDTO){
        ...
        if(user != null){
            ...
            final UserDTO responseUserDTO = UserDTO.builder()
                    .email(user.getEmail())   //정오표보고 수정한것
                    .id(user.getId())
                    .token(token)
                    .build();
